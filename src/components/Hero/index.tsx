        ...
        games && <>
          <div className='h-[400px] grid grid-cols-6 grid-rows-3 w-full gap-2'>
            <div className='bg-slate-600 rounded-[15px] relative overflow-hidden' style={{ gridColumn: '1/5', gridRow: '1/4' }}>
              ...
              <ul className='absolute top-5 left-5 flex items-center gap-3'>
              ...
              <ul className='absolute bottom-5 left-5 flex items-center gap-3'>
              ...
              <div className={`absolute bottom-5 right-5 bg-[#08080848] py-1 px-3 rounded-3xl text-[14px] backdrop-blur-md font-normal  `}>
                {games.rating}
              </div>
      ...
      <div className='w-full py-5 flex justify-center items-center gap-4'>
        {new Array(5).fill(null).map((ele: null, inx: number) => (
          <span key={inx}
            ...
            className={`${num == inx ? 'w-[30px] bg-[var(--color-gradient)]' : ' bg-[var(--bg-icons)] w-[6px]'} cursor-pointer h-[6px] rounded-full transition-all`}></span>
        ))}
      </div>
    </div>
  )
